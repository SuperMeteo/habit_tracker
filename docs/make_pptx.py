...
rect(s, 0.55, 1.9, 11.0, 0.55, TEAL, None, shape=MSO_SHAPE.RECTANGLE)
txbox(s, 11.7, 1.97, 1.0, 0.4, "90%", 20, True, NAVY)
table(s, 0.55, 2.58, 12.23,
      ...
      [["โครงสร้างโปรเจกต์ + ออกแบบฐานข้อมูล 3 ตาราง (Categories / Habits / HabitLogs)",
       ...
       ["ระบบแจ้งเตือนตามเวลาที่ตั้งไว้ (Android)",
        ("รอทดสอบบนเครื่องจริง", AMBER, True), "notification_service.dart · ทดสอบอัตโนมัติ 13 กรณีผ่าน"],
       ["ความถี่แบบ “N ครั้งต่อสัปดาห์”",
        ("เสร็จแล้ว", GREEN, True), "เลือก 1–7 ครั้ง นับผลเป็นรายสัปดาห์ / ทดสอบอัตโนมัติผ่าน"],
       ["จัดการหมวดหมู่ — เพิ่ม / แก้ไข / ลบ (กันลบหมวดที่มี habit ใช้อยู่)",
        ("เสร็จแล้ว", GREEN, True), "manage_categories_screen.dart · ทดสอบฐานข้อมูล 6 กรณี"],
       ["ส่งออกข้อมูลเป็นไฟล์ CSV / JSON",
        ("เสร็จแล้ว", GREEN, True), "data_exporter.dart + share_plus · ทดสอบ 7 กรณี"],
       ["ชุดทดสอบอัตโนมัติ (Unit / Widget test)",
        ("เสร็จแล้ว", GREEN, True), "สั่ง flutter test ผ่าน 45 จาก 45 กรณี"]],
      [6.1, 1.85, 4.28], row_h=0.33, body_size=10.5)
note1(s, 0.55, 6.4, 12.23, "สิ่งที่ต้องใส่",
      "90% : ทุกโมดูลเขียนเสร็จและผ่านทดสอบอัตโนมัติ · เหลือทดสอบแจ้งเตือนบนมือถือจริง และทดลองกับผู้ใช้")
...
table(s, 0.55, 1.45, 12.23,
      ...
      [["T1 ติดตั้งและรันแอป", "สั่ง flutter run บนอีมูเลเตอร์ Pixel 6",
       ...
       ["T7 ชุดทดสอบอัตโนมัติ", "สั่ง flutter test ทั้งโปรเจกต์",
        "ผ่านทุกกรณี", "ผ่าน 45 จาก 45 กรณี", ("ผ่าน", GREEN, True)]],
      [2.35, 3.3, 2.75, 2.33, 1.5], row_h=0.42, body_size=10.5)
...
card(s, 6.68, 4.98, 6.1, 1.68, AMBER, "ข้อค้นพบ",
     ...
     "• ชุดทดสอบอัตโนมัติ 45 กรณีผ่านทั้งหมด รวมทดสอบฐานข้อมูลจริงในหน่วยความจำ\n"
     "• ต้องทดสอบเพิ่ม : แจ้งเตือนบนมือถือจริง และหน้าจอใหม่แบบกดใช้งานจริง",
...
table(s, 0.55, 1.5, 12.23,
      ...
      [["ทดสอบแจ้งเตือนบนมือถือจริง",
        ("โค้ดเสร็จแล้ว", AMBER, True),
        "โค้ดตั้งเวลาเตือนและค่าตั้ง Android เสร็จ ผ่านทดสอบอัตโนมัติ 13 กรณี · ต่อไปติดตั้งบนมือถือ Android 13 ขึ้นไป ตั้งเวลาแล้วรอให้เด้ง",
        "ภาพหน้าจอหรือวิดีโอการแจ้งเตือนที่เด้งตามเวลาที่ตั้ง"],
       ["เก็บหลักฐานหน้าจอฟีเจอร์ใหม่",
        ("ยังไม่เริ่ม", RED, True),
        "จัดการหมวดหมู่ · ส่งออก CSV/JSON · N ครั้งต่อสัปดาห์ ผ่านทดสอบอัตโนมัติแล้ว แต่ยังไม่มีภาพจากการกดใช้งานจริง",
        "ภาพหน้าจอเพิ่มใน docs/screenshots และไฟล์ CSV ที่ส่งออกจริง"],
       ["ไอคอนแอปและรหัสแพ็กเกจ",
        ("ยังไม่เริ่ม", RED, True),
        "ตั้งชื่อแอปเป็น Habit Tracker แล้ว แต่ยังใช้ไอคอนของ Flutter และรหัสแพ็กเกจ com.example ที่ต้องเปลี่ยนก่อนเผยแพร่",
        "ไอคอนของโครงงาน และรหัสแพ็กเกจของตัวเอง"],
       ["ทดลองใช้กับผู้ใช้จริง",
        ("ยังไม่เริ่ม", RED, True),
        "ให้กลุ่มเป้าหมายทดลองใช้แอปต่อเนื่อง แล้วเก็บแบบประเมินความพึงพอใจ",
        "สรุปผลแบบประเมินความพึงพอใจ"],
       ["รายงานฉบับสมบูรณ์",
        ("ยังไม่เริ่ม", RED, True),
        "เรียบเรียงผลการพัฒนา ผลการทดสอบ และผลประเมินจากผู้ใช้",
        "รายงานฉบับสมบูรณ์สำหรับการสอบจบ"]],
...
card(s, 0.55, 1.5, 3.95, 4.15, TEAL, "ระดับความสำเร็จ",
     "ความก้าวหน้าปัจจุบัน : 90%\n\n"
     "ส่วนสำคัญที่ทำสำเร็จ\n"
     "1) บันทึกพฤติกรรมประจำวันลงฐานข้อมูลในเครื่องได้จริง ทั้งแบบทำ/ไม่ทำ แบบมีเป้าตัวเลข และแบบ N ครั้งต่อสัปดาห์\n"
     ...
     "4) จัดการหมวดหมู่ ส่งออกข้อมูล CSV/JSON และแจ้งเตือนตามเวลา")
card(s, 4.72, 1.5, 3.95, 4.15, GREEN, "สิ่งที่พิสูจน์ได้",
     ...
     "• ซอร์สโค้ด 33 ไฟล์ Dart ประมาณ 3,900 บรรทัด อยู่บน Git\n"
     "• ชุดทดสอบอัตโนมัติ 45 กรณี รันผ่านทั้งหมด\n"
...
card(s, 8.89, 1.5, 3.89, 4.15, BLUE, "เป้าหมายภาคเรียนหน้า",
     "• ทดสอบแจ้งเตือนบนมือถือจริง และเก็บภาพหน้าจอฟีเจอร์ใหม่\n"
     "• ออกแบบไอคอนแอปและตั้งรหัสแพ็กเกจของโครงงาน\n"
