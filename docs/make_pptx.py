...
rect(s, 0.55, 1.9, 9.78, 0.55, TEAL, None, shape=MSO_SHAPE.RECTANGLE)
txbox(s, 10.6, 1.97, 2.0, 0.4, "80%", 20, True, NAVY)
table(s, 0.55, 2.58, 12.23,
      ...
      [["โครงสร้างโปรเจกต์ + ออกแบบฐานข้อมูล 3 ตาราง (Categories / Habits / HabitLogs)",
       ...
       ["ชุดทดสอบอัตโนมัติ (Unit / Widget test)",
        ("กำลังดำเนินการ", AMBER, True), "weekly_frequency_test.dart ผ่าน 8/8 กรณี · ส่วนอื่นยังไม่มี"]],
      [6.1, 1.85, 4.28], row_h=0.40, body_size=10.5)
note1(s, 0.55, 6.4, 12.23, "สิ่งที่ต้องใส่",
      "80% : 6 โมดูลหลักเสร็จและสาธิตได้ · ระบบแจ้งเตือนทำไปบางส่วน · ชุดทดสอบอัตโนมัติเริ่มแล้ว 8 กรณี")
...
card(s, 6.68, 4.98, 6.1, 1.68, AMBER, "ข้อค้นพบ",
     ...
     "• ความถี่ N ครั้งต่อสัปดาห์ : สั่ง flutter test แล้วผ่าน 8/8 กรณี\n"
     "• ต้องปรับปรุงก่อนสอบจบ : ระบบแจ้งเตือน และขยายชุดทดสอบอัตโนมัติให้ครอบคลุม",
...
table(s, 0.55, 1.5, 12.23,
      ...
      [["ระบบแจ้งเตือนตามเวลาที่ผู้ใช้ตั้ง",
       ...
       ["ความถี่แบบ “N ครั้งต่อสัปดาห์”",
        ("เสร็จแล้ว", GREEN, True),
        "เพิ่มตัวเลือก 1–7 ครั้ง · นับครั้งที่ทำในสัปดาห์เทียบเป้า · Streak นับเป็นสัปดาห์ ทั้งหน้าหลักและหน้าสถิติ",
        "ผลทดสอบ : ตั้งเป้า 3 ครั้ง/สัปดาห์ ทำได้ 2 ครั้ง แสดง 2/3 — ผ่านแล้ว"],
       ["หน้าจัดการหมวดหมู่",
       ...
       ["ชุดทดสอบอัตโนมัติ",
        ("เริ่มแล้ว", AMBER, True),
        "มีชุดทดสอบความถี่รายสัปดาห์ 8 กรณีแล้ว · ต่อไปเขียนทดสอบ Streak แบบเจาะจงวัน และหน้าหลัก แทนไฟล์ตัวอย่างเดิม",
...
card(s, 4.72, 1.5, 3.95, 4.15, GREEN, "สิ่งที่พิสูจน์ได้",
     ...
     "• ซอร์สโค้ด 29 ไฟล์ Dart ประมาณ 3,200 บรรทัด อยู่บน Git\n"
     "• ชุดทดสอบอัตโนมัติ 8 กรณี รันผ่านทั้งหมด\n"
...
card(s, 8.89, 1.5, 3.89, 4.15, BLUE, "เป้าหมายภาคเรียนหน้า",
     "• ทำระบบแจ้งเตือนตามเวลาให้ใช้งานได้จริง\n"
     "• เพิ่มหน้าจัดการหมวดหมู่ และการส่งออก/สำรองข้อมูล\n"
     "• ขยายชุดทดสอบอัตโนมัติให้ครอบคลุมทุกส่วน\n"
